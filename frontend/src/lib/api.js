...
export const api = {
  ...
  // Accounts
  listAccounts: (search = "", account_type = "") => {
    const p = new URLSearchParams()
    if (search)       p.set("search",       search)
    if (account_type) p.set("account_type", account_type)
    const qs = p.toString()
    return request(`/accounts${qs ? `?${qs}` : ""}`)
  },
  listTypes:     ()        => request("/accounts/types"),
  ...
  wipeAccounts:  ()        => request("/accounts",       { method: "DELETE" }),
  bulkImport:    (accounts)=> request("/accounts/bulk-import", { method: "POST", body: { accounts } }),
}
